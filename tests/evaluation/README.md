...
預設 dry-run，檢查 schema、切分與每張完整解碼，列出待人工覆核 caseId；不讀取 key、不呼叫 Provider。一般 npm test/CI 也不呼叫 AI。
...
呼叫數預檢與每次 Provider 無 retry；工具保守地以「模型整個 context 的未快取 input 價格＋2400 output tokens」保留每次費用，超預算就拒絕，沒有無上限重跑。這是本次工具執行的保守預檢，不是帳號級硬支出限制，也不涵蓋其他使用者呼叫。費率查核日 2026-09-04：input $0.40／1M，output $1.60／1M；實際執行前應重核官方價格與帳號機制。[官方模型與價格](https://developers.openai.com/api/docs/models/gpt-4.1-mini)
